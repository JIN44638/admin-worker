  ...
  <div class="w-full">
    ...
    <div class="grid grid-cols-7 gap-1">
      <div
        ...
        <span v-if="countJobs[d.key]"
        class="mt-1 text-[10px] px-1.5 py-0.5 rounded-full bg-blue-50 dark:bg-blue-900/50 text-blue-700 dark:text-blue-300">{{countJobs[d.key]}}건</span>
      </div>
    </div>
    <!-- 선택 날짜 일정 리스트 -->
     <div class="mt-4">
     ...
     <ul class="space-y-2">
      <li v-for="job in filteredJobsSelected" :key="job.id"
         ...
         <div
            class="w-9 h-9 rounded-lg flex items-center justify-center text-xs font-bold"
             :class="
              ...
              ">
          {{ job.type === "luggage" ? "수" : "제"}}</div>
          <div class="flex-1 min-w-0">
            <p class="text-sm font-semibold truncate text-gray-900 dark:text-white">{{ job.customerName }} 
             ...
             <p class="text-xs text-gray-600 dark:text-gray-400 truncate">
              ...
              <div class="mt-2 flex items-center gap-1.5">
                ...
                :class="statusClass(job.status)">{{ statusText(job.status) }}</span>
                <span v-if="job.memo"
       ...
       <div class="mt-3 grid grid-cols-3 gap-2 text-xs">
        ...
        <a :href="mapLink(job.address)" target="_blank" class="py-2 rounded-lg bg-gray-100 text-gray-700 text-center">길찾기</a>
...
// jobs.json에 넣은 더미데이터 불러오기 위해 연결함(위에 import해야함)
...
// 날짜 key 포맷 함수 padStart(2 , "0")= 2를 0부터 시작하겠다
function fmtKey(d) {
...
function dayClass(d){
  ...
  return [
    d.outside ? "text-gray-400 dark:text-gray-600"
      : "text-gray-900 dark:text-gray-100",
      isToday ? "ring-1 ring-blue-500 dark:ring-blue-400" : "",
      isSelected  ? "bg-blue-600 dark:bg-blue-500 text-white"
      : "bg-white dark:bg-gray-800", 
  ]
}
// 이전 클릭(달력) - 1뺀 달력이 1이 되어서 나온다(=이전달력)
function prevMonth(){
...
function mapLink(address){
  // encodeURIComponent() = 문자열을 URL 인코딩하는 매서트
