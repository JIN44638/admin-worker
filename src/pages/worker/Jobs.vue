        ...
        <div class="flex items-start gap-3">
          ...
          <div class="flex-1">
            <p class="font-semibold text-base flex items-center justify-between text-gray-900 dark:text-white">
              <span
                >{{ job.storeName }} <span class="text-gray-400 dark:text-gray-500"> - {{ job.time }}</span></span
              >
            </p>
...
function gotoWorkDetail(job) {
  ...
  // 라우터(index.js)에도 디테일 페이지 넘어가는거 별도 설정해야함  { path: "detail/:id?", component: Detail, name: "WorkerDetail" },
  // 아이디 별로 페이지 이동해야함
  // 이동하면 주소창에 ?jobId=1(여기서 숫자는 id임)
  router.push({
    name: "WorkerDetail",
    query: { jobId: job.id },
  });
