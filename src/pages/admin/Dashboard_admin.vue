...
const filteredReservations = computed(() => {
  let result = [...reservations.value]; //예약 목록 복사 한것을 담다
  // 날짜를 필터링
...
const dashFilterFn = (data, filters) => {
  ...
  if (filters.serviceType && filters.serviceType !== "all") {
    return result.filter((reser) => reser.type === filters.serviceType);
  }
  // 접수상태 필터링
  if (filters.receiptStatus && filters.receiptStatus !== "all") {
    return result.filter((reser) => reser.status === filters.receiptStatus);
  }
  return result;
};
</script>
