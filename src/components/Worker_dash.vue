<template>
  <!-- 기사 현황(공통컴포넌트 사용) -->
  <!-- 각 페이지 마다 검색창안에 placeholder 달라서 다르게 설정하기 위해서 search-placeholder(사용함 대문자 사용하면 안됨)  -->
  <SearchTable
    :data="Workers"
  ...
  />
  <!--기사 상세 모달-->
  <!-- closeModal 전체에 넣어서 모달 안에서 클릭시 창이 전부 닫혀서 안에 div에  @click.stop 설정하면 배경에만 클릭시 모달 닫기게 설정됨 -->
  <div
    v-if="selectedWorker"
    @click="closeModal"
...
// 기사목록 더미데이터 = 외부 파일에서 가져오기-import해야함(worker.js에 더미 저장)
const Workers = ref([...workersData]);
// 처음에 안보이게 할려고 null 값부여함(상세정보 페이지 모달창임)
